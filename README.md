...
# BNMR-HTMRe #
##############
fit <- bayesnmr(df$y, df$sd, x, df$ids, df$iarm, df$npt, list(c(0:3)), prior = list(c01=1.0e05, c02=4, nu=3), mcmc=list(ndiscard=2500,nskip=1,nkeep=10000), init = list(beta = c(beta_true, gamma_true), sig2 = sig2_true))
...
* `mcmc` - list of MCMC-related parameters: number of burn-ins(nwarmup), number of thinning(nskip), and posterior sample size(nsave)
